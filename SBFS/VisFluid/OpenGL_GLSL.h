#ifndef __OPENGL_GLSL_H__
#define __OPENGL_GLSL_H__
#include <GL/glew.h> 
#include <GL/glut.h> 
#include <stdio.h>
...
GLuint Setup_GLSL(const char *name)
{
	...
	else printf("Succeeded in creating shader program object.\n");
	if (!(vertexShaderObject = glCreateShader(GL_VERTEX_SHADER)))
	{
		printf("Error creating vertex shader object.\n");
		exit(1);
	}
	else printf("Succeeded in creating vertex shader object.\n");
	if (!(fragmentShaderObject = glCreateShader(GL_FRAGMENT_SHADER)))
	{
		printf("Error creating fragment shader object.\n");
		exit(1);
	}
	else printf("Succeeded in creating fragment shader object.\n");

	//Step 2: Load the shaders from the disk into the two shader objects
	...
	//Step 3: Compile the vertex shader 
	...
	//Step 5: Attach the shader objects and link the program object
