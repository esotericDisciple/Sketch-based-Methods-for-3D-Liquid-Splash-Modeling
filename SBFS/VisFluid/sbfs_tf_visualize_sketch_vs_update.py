...
def get_data_addrs_cur_frame(data_x_addrs, data_y_addrs, cur_frame):
	if os.path.isfile(data_x_addrs[cur_frame]) and os.path.isfile(data_y_addrs[cur_frame]):
		return data_x_addrs[cur_frame], data_y_addrs[cur_frame]
	else:
		print('{:} or {:} does not exists!'.format(data_x_addrs[cur_frame], data_y_addrs[cur_frame]))
...
def surface_reconstruction_from_levelset_update(threshold_value, result_dir, mat_dir, manta_dir, sketch_addr, lvst_addr):
	addr_lvst 			= lvst_addr
	filename_lvst 		= addr_lvst[int(addr_lvst.rfind(os.sep)+1):int(addr_lvst.rfind('.'))]
	frame_num 			= filename_lvst[int(filename_lvst.rfind('_'))+1:]
	addr_lvst_pre 		= addr_lvst[0:int(addr_lvst.rfind(os.sep))]
	...
	# load the original predict levelset
	outpath_pred_lvst 	= os.path.join(result_dir, seed_num+'#'+simname+'#'+filename_lvst+'_pred')
	predict_lvst = np.load(outpath_pred_lvst+ '.npy')
...
if __name__ == '__main__':
	...
	parser.add_argument('--cur_frame', required=True, type=int)
	...
	sketch_addr, lvst_addr = get_data_addrs_cur_frame(test_addrs_sketch, test_addrs_lvst, args.cur_frame)
	surface_reconstruction_from_levelset_update(args.threshold_value, args.result_dir, args.mat_dir, args.manta_dir, sketch_addr, lvst_addr)