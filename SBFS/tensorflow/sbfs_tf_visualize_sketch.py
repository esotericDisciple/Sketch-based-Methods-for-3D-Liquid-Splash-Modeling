...
# uni head definition
header_example, content_example = uniio.readUni(os.path.join('./data', 'flipLevelSet_example.uni'))
...
if __name__ == '__main__':
	...
	filetype    	= 'test'
	test_addrs_sketch, test_addrs_lvst = load_data_addrs(filetype)
	for ibatch in range(num_batch):
		sketch_addrs, lvst_addrs  = get_data_addrs_next_batch(test_addrs_sketch, test_addrs_lvst, ibatch, batch_size)
		surface_reconstruction_from_levelset(threshold_value, result_dir, mat_dir, manta_dir, ibatch, batch_size, iepoch, filetype, sketch_addrs, lvst_addrs)